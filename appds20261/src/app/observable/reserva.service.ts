...
import { GenericResponse, PageResponse, ReservaResponse, SuggestionResponse, toGenericResponse } from '../shared/models';
...
export class ReservaService {
  ...
  search(valor: string): Observable<GenericResponse<ReservaResponse[]>> {
  ...
  verificarDisponibilidad(habitacionId: number, fechaEntrada: string | Date, fechaSalida: string | Date): Observable<GenericResponse<boolean>> {
    const fe = fechaEntrada instanceof Date ? this.formatDate(fechaEntrada) : fechaEntrada;
    const fs = fechaSalida instanceof Date ? this.formatDate(fechaSalida) : fechaSalida;
    return this.http.get<any>(
      `${this.API}/checkavailability?habitacionId=${habitacionId}&fechaEntrada=${fe}&fechaSalida=${fs}`
    ).pipe(

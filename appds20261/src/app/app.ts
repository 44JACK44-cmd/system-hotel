...
export class App implements OnInit, OnDestroy {
  ...
  constructor() {
    effect(() => {
      ...
      document.title = c.nombre ? `${c.nombre} — ${c.ciudad || 'Sistema Hotelero'}` : 'Sistema Hotelero';
      const link = document.querySelector('link[rel="icon"]') as HTMLLinkElement;
      if (link) link.href = c.logo || 'favicon.ico';
    });
  }
  ...
  centroAyuda(): void {
    ...
    this.messageService.add({ severity: 'info', summary: 'Centro de Ayuda',
      detail: `Bienvenido a ${nombre}. Use el menú lateral para navegar.${tel}${email}`,
      life: 8000 });
  }

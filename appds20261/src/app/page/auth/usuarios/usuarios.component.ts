  ...
  save(): void {
    ...
    if (this.editing && this.editingId) {
      this.usuarioService.actualizar(this.editingId, data).subscribe({
        next: () => { this.messageService.add({ severity: 'success', summary: 'Exito', detail: 'Usuario actualizado' }); this.dialogVisible = false; this.layoutState.setOverlay(false); this.loading = false; this.loadUsuarios(); this.loadStats(); this.estadoActualizacion.usuarioCambio(); },
      ...
      this.usuarioService.crear(data).subscribe({
        next: () => { this.messageService.add({ severity: 'success', summary: 'Exito', detail: 'Usuario creado' }); this.dialogVisible = false; this.layoutState.setOverlay(false); this.loading = false; this.loadUsuarios(); this.loadStats(); this.estadoActualizacion.usuarioCambio(); },
        error: (err) => { this.loading = false; this.messageService.add({ severity: 'error', summary: 'Error', detail: err.error?.message || 'Error' }); }
      });
    }
  }
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.usuarioService.cambiarEstado(u.id).subscribe({
          next: () => { this.messageService.add({ severity: 'success', summary: 'Exito', detail: 'Estado cambiado' }); this.loading = false; this.loadUsuarios(); this.loadStats(); this.estadoActualizacion.usuarioCambio(); },
          error: (err) => { this.loading = false; this.messageService.add({ severity: 'error', summary: 'Error', detail: err.error?.message || 'Error' }); }
        });
      }
    });

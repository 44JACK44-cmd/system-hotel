...
public class ReservaController {
    ...
    public ResponseEntity<List<SuggestionResponse>> actionSuggestions(@RequestParam String termino) {
        List<SuggestionResponse> list = reservaBusiness.buscarSugerencias(termino);
        return ResponseEntity.ok(list);
    }

    @GetMapping("search")
    public ResponseEntity<ResponseReserva> actionSearch(@RequestParam String valor) {
        List<ReservaResponse> list = reservaBusiness.searchConfirmadas(valor);
        ResponseReserva response = new ResponseReserva();
        response.success();
        response.setListReserva(list);
        return ResponseEntity.ok(response);
    ...
    public ResponseEntity<ResponseReserva> actionCheckAvailability(
        ...
        if (fechaEntrada == null || fechaSalida == null ||
                fechaEntrada.isBlank() || fechaSalida.isBlank()) {
            response.listMessage.add("Las fechas son obligatorias");
            return ResponseEntity.badRequest().body(response);
        }
        try {
            LocalDate fe = LocalDate.parse(fechaEntrada);
            LocalDate fs = LocalDate.parse(fechaSalida);
            boolean disponible = reservaBusiness.verificarDisponibilidad(habitacionId, fe, fs);
            response.success();
            response.setDisponible(disponible);
            return ResponseEntity.ok(response);
        } catch (java.time.format.DateTimeParseException e) {
            response.listMessage.add("Formato de fecha invalido. Use yyyy-MM-dd");
            return ResponseEntity.badRequest().body(response);
        }
    }
}
